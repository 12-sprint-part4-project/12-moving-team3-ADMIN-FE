...
import type { AdminMemberDetailQuery } from '@/types/adminMember';
...
export const useAdminMemberDetail = (
  ...
  useQuery({
    queryKey: ADMIN_MEMBER_QUERY_KEYS.detail(memberId ?? null, options?.query),
    queryFn: () => {
      if (!memberId) {
        return Promise.reject(new Error('memberId is required'));
      }

      return getAdminMemberDetail(memberId, options?.query);
    },
